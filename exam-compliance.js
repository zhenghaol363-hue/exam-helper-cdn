// exam-compliance.js — 2026年平邑合规知识测试题库密钥（本地哈希表，FNV-1a）
// 用法：练习页地址栏粘贴：
...
(function(){
  if (window.__EXAM_COMPLIANCE_RUN__) return;
  window.__EXAM_COMPLIANCE_RUN__ = true;
  ...
  // FNV-1a 32位哈希（分步乘法避免JS浮点溢出，对中文与Python端一致）
  function mul32(x, y) {
  ...
  function doFill(){
    ...
    if (!panels.length) {
      // 兼容新版DOM
      panels = Array.from(document.querySelectorAll('.question-panel'));
    }
    var hit = 0, miss = 0;
    ...
    panels.forEach(function(p){
      var inputs = Array.from(p.querySelectorAll('input[type=radio], input[type=checkbox]'));
      if (inputs.length && !inputs.some(function(x){ return x.checked; })) {
        inputs[0].click();
      }
    });
  ...
  setTimeout(function(){
    var r = doFill();
    console.log('[compliance] 命中', r.hit, '/', r.total, '未命中', r.miss);
    setTimeout(function(){
      try {
